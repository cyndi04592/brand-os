...
// ── 反 AI 美學 Prompt 配方庫 ─────────────────────────────
// ═══════════════════════════════════════════════════════════════
//  🆕 v3.41(2026-08-24)🚫 隱藏「幫既有角色多拍造型」下拉 —— 實測 seed 鎖不住臉,
//     三張全是不同人。改走下方「多角度人物表 · Kontext 鎖臉」(編輯真圖,不重畫)
  ...
  return `
    <div class="kai-panel" id="kai-panel">
      ...
      <!-- ═══════════════════════════════════════════════════════════
           ...
           實測結論(RA 用又晴跑的):
             ・seed 有正確帶入(825079427)、參數也對(28 歲 / female)
             ・生出來的三張【完全不同人】,而且跟又晴本人也不像
           原因:seed 只鎖「隨機起點」,不鎖「臉」。
             提示詞裡任何一個字不同(服裝 / 場景 / 佈光),就是另一個人。
             這與既有結論一致:nanobanana compose / face_swap / flux-pulid /
             ideogram-character 全試過,全都生出「不同的人」。
           ★ 所以這條路的核心價值(同一個人換造型)技術上做不到。
             留著只會讓客戶生出一堆不像自己 KOL 的照片,
             選進 Look 之後影片就崩 —— 比沒有這個功能更糟。
           ★ 正解是下面那塊「多角度人物表 · Kontext 鎖臉」——
             那是【編輯真圖換角度】,不是照文字重畫,才會是同一個人。

           ⚠️ 用 display:none 隱藏,【不刪】:
             #kai-persona 在建立新角色流程裡有五處引用 ——
             syncBrandAndLoadPersonas 填選項、onPersonaChange 讀值、
             建立成功後自動選中新角色。刪掉那條流程會整個壞。
             以後 Kontext 那條成熟了要拿回來,只要拿掉這行 style。
           ═══════════════════════════════════════════════════════════ -->
      <div class="kai-row-persona" style="display:none">
        <div class="kai-field">
...
function updateFolderHint() {
  ...
  // ══════════════════════════════════════════════════════════════════
  //  🩹 2026-09-08(第三版)· 標籤要看【這位角色的狀態】,不是只講區塊功能
  //  ────────────────────────────────────────────────────────────────
  //  ⚠️ 2026-08-22:第一版印的是 Google 雲端資料夾路徑(那條路早就不存在)。
  //  ⚠️ 2026-09-08 上午:第二版改成「🎨 正在幫「XXX」生形象照」——
  //    但 RA 立刻指出:沐恩早就有形象照了,還跟她說「正在幫她生」,
  //    等於催客戶做一件已經做完的事,看起來很不智能。
  //  ★ 第三版:已經有照片 → 灰字、講【只能重生一張新的臉】;
  //    還沒有 → 橘字、催他生第一張。
  //  ⚠️ 判斷用 kolSetupSteps()(kol.html 掛在 window 上)——
  //    跟形象庫卡片、底部進度同一個標準。三個地方講不同的話最傷信任。
  //    抓不到那支時退回舊文案,不能因為模組沒載到就整條空白。
  // ══════════════════════════════════════════════════════════════════
  hint.style.display = '';
  let _hasPhoto = false;
  try {
    const _k = (S.personas || []).find(x => String(x.persona_name || '').trim() === persona);
    _hasPhoto = !!(_k && String(_k.talking_photo_id || '').trim());
  } catch (_) {}

  if (_hasPhoto) {
    //  ⚠️ 【不要】寫成「再生一張換造型」——
    //     衣服是拍片時由服裝師(kol-wardrobe.js)決定的,不是靠換照片。
    //     寫成換造型會害客戶為了換裝來這裡重生 → 生出來是【另一張臉】,
    //     那是 seed 只鎖隨機起點、不鎖臉的老問題(2026-08-24 已實測)。
    //     這一區對已完成的角色,用途只剩「對這張臉不滿意,重生一張」。
    hint.className = 'kai-folder-hint idle';
    hint.textContent = '「' + persona + '」的形象照已完成 · 這裡只用來重生一張新的臉(衣服在拍片時決定)';
  } else {
    hint.className = 'kai-folder-hint';
    hint.textContent = '🎨 幫「' + persona + '」生第一張形象照';
  }
}
