...
window.KolStitch = (function () {
  ...
  async function pollEpisode(reqId, brandId, onTick, endpoint, maxTries) {
    ...
    for (let i = 0; i < _tries; i++) {
      try {
        ...
        // ═══════════════════════════════════════════════════════════
        //  🛟 2026-09-12 v6.33:主動問上游【拿到成品就直接用】
        //  ★ 病(RA 2026-09-12 實例):PiAPI 09:23:57 就顯示「完成的」,
        //    前端卻一路空等到 20 分鐘逾時。RA 手動打一次 fal_poll,
        //    立刻回 COMPLETED + videoUrl —— 東西一直都在,只是沒人去拿。
        //  ★ 病因:舊版每 4 次確實會問上游,但【只讀 status 拿去顯示文字】,
        //    把回傳裡的 videoUrl 丟掉。等於拿到鑰匙卻沒開門。
        //    webhook 一掉,episode_result 永遠 pending → 撐滿 240 次。
        //  ★ 修法:同一個呼叫,多看一眼有沒有成品,有就直接 return。
        //    webhook 正常時行為不變(episode_result 會先回),
        //    webhook 掉包時這條救得回來。零額外成本。
        // ═══════════════════════════════════════════════════════════
        if (endpoint && i % 4 === 0) {
          try {
            ...
            const rescued = (fp && (fp.videoUrl || fp.video_url || fp._url)) || '';
            if (rescued && raw === 'COMPLETED') {
              _dbg('[KolStitch] 🛟 webhook 沒回來,改從上游直接取回成品 →', rescued);
              return rescued;
            }
          } catch (_) {}
        }
    ...
    if (shared && shared.front) {
      ...
      bodyB += '\n\n'
        + ((_sg && _sceneMode !== 'photo')   // 🏕 v6.80:客戶原照走下面那句(保留照片裡的人)
           ...
           //  🧍 v6.79(2026-09-18)RA 實測:「我提供的照片本身有人啊?」——
          //   舊句寫死「這張只給空的空間、裡面的活動不鎖」,那是為【九宮格空間庫】寫的
          //   (那些圖確實是空的);但【客戶自己上傳的單張實景照】被套進同一句 →
          //   模型被告知「人不算數」→ 照片裡明明有人也被清掉,生出空無一人的營地/店面
          //   = 打烊感,正是當初要治的樣品屋病。
          //  ★ 改成一句事實:照片裡有什麼就是什麼,包括裡面的人。
          //  ★ v6.79b RA 立刻抓到:第一版寫「遠而失焦」是把所有人壓成同一層 ——
          //    等於一排背景貼紙,沒有前後景深,反而更假。而且照片裡的人本來就
          //    各在不同距離,硬說「全部遠而失焦」是在覆蓋照片的事實。
          //  ★ v6.79c RA:「精簡後很爛」—— 上一版寫成「維持照片給他的距離」,
          //    聽起來對,但那是廢話:沒有任何一個字告訴模型【每一層長什麼樣】。
          //    → 把三層寫清楚(這是有資訊的字,不是贅字):
          //      近 = 清楚、會動,手或肩可能掠過畫面邊緣
          //      中 = 稍微失焦但認得出在做什麼
          //      遠 = 只剩逆光的輪廓
          //    三層一寫,景深才立得起來;全部壓成「遠而失焦」就是一排背景貼紙。
          //  ★ 鐵律沒鬆動:照片裡【沒有】的人依舊不會被生出來 ——
          //    這句只說「照著這張」,沒有任何一個字叫模型加人。
          : '[SCENE_IMG] = location (same background and layout; do not rearrange). Whatever is in that photograph is what is there, including its people, sitting at three depths: the nearest ones stay sharp and keep moving, and a hand, shoulder or back may cross the edge of frame; the ones at middle distance are soft but still readable — you can tell what they are doing; the ones deep in the space are only shapes against the light. None of them look at the lens. ')
  ...
  async function _acquireShared(tag) {
    ...
    for (let i = 0; i < 90; i++) {                       // 最多等 30 分鐘
      ...
      if (!told) {
        told = true;
        //  🤐 v6.78:白牌 —— 不得顯示容量數字,一律用 Worker 給的說法
        const _m = (r && r.message) || '目前算力機滿載,排隊中…前面完成後會自動接上(不會扣點)';
  ...
  async function _acquireSlot(tag) {
    ...
    while (_inFlight >= _maxInflight()) {
      if (!told) {
        ...
        const _m = '目前算力機滿載,' + (tag || '這一段') + '排隊中…前面完成後會自動接上(不會扣點)';
        try { if (_uiLog) _uiLog('⏳ ' + _m); } catch (_e) {}
        _dbg('[KolStitch] 🚪 ' + _m);
      }
      await _sleep(3000);
  ...
  async function generateSegment(opts, onTick) {
    ...
    if ((opts.provider || 'piapi') === 'piapi' && opts.shared && opts.shared.front) {
      ...
      function fitRules(tailStr, budget) {
        ...
        const rules = String(tailStr || '')
          ...
          .filter(Boolean);
        const kept = [];
        const dropped = [];   // 🩳 2026-08-23:記下被丟掉的,印出來給人看
        let used = 0;
        for (let i = 0; i < rules.length; i++) {
          ...
          if (used + cost > budget) { dropped.push(rules[i]); continue; }   // 整條跳過,不切半句
    ...
    //  🍽 v6.77:整支已經訂好位(bookedSlot)→ 直接用那張號碼牌,不再自己排隊
    const _mySlot = opts.bookedSlot
      ? opts.bookedSlot
    ...
    // 🛟 v6.33:補上 endpoint —— 沒帶的話 pollEpisode 連「主動問上游」那條
    //    備援都不會啟動,只能純等 webhook。這就是 2026-09-12 第二段
    //    明明生完卻空等到逾時的直接原因。
  ...
  async function runStitchFlow(plan, opts) {
    ...
    //  🍽 v6.77(2026-09-18)【訂位制:全有全無】RA:「像訂位一樣」
    //   六段是一支片,少一段整支報廢 —— 所以不能六段各自排隊、先跑一半。
    //   ★ 整支開跑前一次要 N 個位子:湊得齊一次發、湊不齊整支等。
    //   ★ 拿到的號碼牌分給每一段,段落跑完各自還;沒拿到(Worker 連不上等)就退回舊行為。
    let _bookedSlots = [];
    try {
      const _need = chunks.length;
      for (let t = 0; t < 90 && !_bookedSlots.length; t++) {          // 最多等 30 分鐘
        const r = await api('gate_acquire', { pool: 'video', need: _need, ttlSec: 1800,
                                              tag: (opts.kolName || '') + ' ' + _need + ' 段' });
        if (r && r.ok && Array.isArray(r.slots) && r.slots.length) {
          _bookedSlots = r.slots;
          _dbg('[KolStitch] 🍽 訂位成功 ' + _need + ' 段 → ' + r.used + '/' + r.limit);
          break;
        }
        if (!r || r.code !== 'GATE_FULL') { _dbg('[KolStitch] 🍽 訂位無回應,退回逐段模式'); break; }
        if (t === 0) {
          try { if (_uiLog) _uiLog('⏳ ' + ((r && r.message) || '目前算力機滿載,排隊中…會自動接上(不會扣點)')); } catch (_e) {}
        }
        //  每 2 分鐘回報一次還在排隊,客戶才知道系統沒當機
        if (t > 0 && t % 6 === 0) { try { if (_uiLog) _uiLog('⏳ 仍在排隊…已等 ' + Math.round(t * 20 / 60) + ' 分鐘,輪到就會自動開始'); } catch (_e) {} }
        await _sleep((r.waitSec || 20) * 1000);
      }
    } catch (e) { _dbg('[KolStitch] 🍽 訂位失敗,退回逐段模式:' + e.message); }
    ...
    const tasks = chunks.map(function (chunk, i) {
      ...
      return generateSegment({
        ...
        bookedSlot: _bookedSlots[i] || '',         // 🍽 v6.77 訂位號碼牌:有就不用再各自排隊
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  🧯 2026-09-12 v6.33:Promise.all → allSettled(一段掛掉不要全毀)
    //  ★ 病:舊版只要任一段 throw,整個流程立刻中斷,
    //    composeSegments 那行【永遠不會被執行】—— 所以 fal 後台查無紀錄,
    //    但前端顯示的是「接片失敗」,訊息完全誤導方向(RA 因此懷疑接片壞掉)。
    //  ★ 代價更痛:兩段都已經生成成功、都已經付錢,客戶卻什麼都拿不到。
    //  ★ 修法:改 allSettled,成功的段先留著;
    //    只有【全部都掛】才整個失敗,否則有幾段就接幾段。
    //  ★ 錯誤訊息也一併改成講實話:是哪一段沒回來,不要再叫「接片失敗」。
    // ═══════════════════════════════════════════════════════════════════
    const _settled = await Promise.allSettled(tasks);
    const segments = [];
    const _lostIdx = [];
    _settled.forEach(function (r, i) {
      if (r.status === 'fulfilled' && r.value && r.value.url) segments.push(r.value);
      else _lostIdx.push(i + 1);
    });
    if (!segments.length) {
