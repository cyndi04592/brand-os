...
const LOCATIONS = {
  ...
  async function generateSceneGrid(ctx) {
    ...
    // ═══════════════════════════════════════════════════════════════
    //  🗜 v5.29(2026-09-15)g528 → g529:讓舊的九宮格重生一次,吃到 Worker v5.33 的壓縮。
    //   病灶(RA 2026-09-15 實測):舊的九宮格 6.10 MB,算力機從我們的 CDN 抓圖時逾時
    //   ('Failed to download media … read body err: context deadline exceeded'),
    //   而且每一段都要抓一次 → 同一批同樣的圖,一段成功兩段失敗。
    //   Worker v5.33 在存進 R2 之前加了壓縮(寬 1600、品質 85,約 1.5 MB),
    //   但舊圖已經在 R2 裡了 —— 版本號 +1 是讓它重生一次的唯一方法。
    //  ⚠️ 以後只要動到九宮格提示詞、或改了存圖的處理方式,GRID_VER 就要 +1。
    const GRID_VER = 'g529';   // ← 改九宮格提示詞時才 +1(g528 → g529 …)
