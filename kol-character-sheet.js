/* kol-character-sheet.js · v0.3 — 多角度人物表(Kontext 鎖臉轉頭)
   依賴 window.KAI(kol-ai-generator v3.29+ 提供 WORKER_URL / PASSWORD / S)
...
(function () {
  'use strict';
  var VER = 'v0.3-kontext';

  function K() { return window.KAI || null; }

  var FRONT = null;                                   // 選定的正臉 url
  ...
  function build() {
    ...
    box.appendChild(el('div', 'font-size:12px;color:#9a9aa8;line-height:1.6;margin-bottom:10px;',
      '挑一張滿意的<b>正臉</b> → 一鍵用 Kontext「鎖臉轉頭」生 3/4 + 側臉。<br>是<b>編輯真圖換角度</b>、不是重生成 → 不磨皮、不換人。三張同一張臉可餵 Seedance 鎖人。'));
    ...
    var btnUse = el('button', btnCss('#1f9d57', true), '✅ 用這組當人物表');
    ...
    btnUse.onclick = function () {
      window.KOL_CHARACTER_SHEET = { front: RESULTS.front, threeQuarter: RESULTS.q34, profile: RESULTS.profile, ts: Date.now() };
      status.textContent = '✅ 已存人物表 → window.KOL_CHARACTER_SHEET(之後餵 Seedance 鎖人用)。';
    };
  }
  ...
  console.log('[character-sheet] 🎬 ' + VER + ' 就緒(Kontext 鎖臉)');
})();
